...
func InitStatic(engine *gin.Engine) error {
	return StaticFile(Static(), func(fileMap map[string][]byte) {
		indexHTML, ok := fileMap["nav-rain-grid-web/browser/index.html"]
		if !ok {
			panic("nav-rain-grid-web/browser/index.html 文件不存在")
		}

		for fileKey, fileBytes := range fileMap {
			fileKey := fileKey
			fileBytes := fileBytes
			ginStaticFilePath := strings.TrimPrefix(fileKey, "nav-rain-grid-web/browser/")
